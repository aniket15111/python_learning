...
def start_clicking():
    ...
    def click_loop():
        while clicking:
            i = 0
            while i < len(click_points):
                if not clicking:
                    break

                delay = click_points[i][2] / 1000.0
                # Start a group of simultaneous clicks
                group_threads = []
                while i < len(click_points) and click_points[i][2] == 0:
                    x, y, _ = click_points[i]
                    t = threading.Thread(target=pyautogui.click, args=(x, y))
...
def pick_position():
    ...
    click_points.append([pos.x, pos.y, 0])
    update_point_list()
    show_markers()
...
def update_point_list():
    ...
    for i, pt in enumerate(click_points):
        point_listbox.insert(tk.END, f"{i + 1}. X: {pt[0]}, Y: {pt[1]}, Delay: {pt[2]}s")
...
def make_marker_draggable(win, index):
    ...
    def on_drag(event):
        ...
        y = win.winfo_y() + dy
        win.geometry(f"+{x}+{y}")
        click_points[index][0] = x
        click_points[index][1] = y
        update_point_list()
...
def show_markers():
    clear_markers()
    for i, (x, y, _) in enumerate(click_points):
        win = tk.Toplevel(root)
        win.overrideredirect(True)
        win.attributes("-topmost", True)
        win.attributes("-transparentcolor", "red")
